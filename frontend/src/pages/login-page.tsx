  ...
  return (
    <main className="relative grid min-h-dvh place-items-center overflow-hidden bg-[radial-gradient(circle_at_12%_10%,rgba(45,212,191,0.18),transparent_30%),linear-gradient(135deg,#dfeae7,#f6f7f4_55%,#d8e6e3)] px-4 py-8">
      ...
      <section
        ...
        aria-label="JuriGPT authentication"
      >
        <motion.div
        ...
        >
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={registerMode ? "register-panel" : "login-panel"}
            ...
            >
              ...
              <button
                type="button"
                onClick={switchMode}
                className="mt-8 rounded-full border border-white/50 px-8 py-3 text-xs font-semibold uppercase tracking-[0.14em] transition hover:bg-white hover:text-teal-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
              >
        ...
        >
          <div className="w-full max-w-sm">
            <div className="mb-8 flex items-center gap-3 md:hidden">
              ...
              <div>
                <p className="font-semibold text-slate-900">JuriGPT</p>
                <p className="text-[10px] uppercase tracking-[0.16em] text-teal-700">
                  Legal intelligence
                </p>
              </div>
            </div>
            <AnimatePresence mode="wait">
              <motion.div
                key={registerMode ? "register-form" : "login-form"}
                initial={{ opacity: 0, x: registerMode ? 12 : -12 }}
              ...
              >
                ...
                {error ? (
                  <div
                    className="mb-5 flex items-start gap-2 rounded-2xl border border-rose-100 bg-rose-50 px-4 py-3 text-xs leading-5 text-rose-700"
                    role="alert"
                  >
                ...
                <form onSubmit={submit} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 hover:text-teal-700"
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-teal-700 px-5 text-sm font-semibold text-white shadow-[0_14px_30px_-16px_rgba(15,118,110,0.7)] transition hover:-translate-y-0.5 hover:bg-teal-600 disabled:pointer-events-none disabled:opacity-60"
                  >
                    {submitting ? (
                ...
                <p className="mt-6 text-center text-xs text-slate-500 md:hidden">
                  {registerMode ? "Already have an account?" : "New to JuriGPT?"}{" "}
                  <button
                    type="button"
                    onClick={switchMode}
  ...
  return (
    <label className="relative block">
      ...
      <input
        ...
        minLength={minLength}
        placeholder={label}
        className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50/70 pl-11 pr-11 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-teal-400 focus:bg-white focus:ring-2 focus:ring-teal-100"
      />
    </label>
  );
}
